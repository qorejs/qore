...
Qore can be used inside React without replacing your UI stack. The preview adapter lives in [`packages/react`](./packages/react) and subscribes to Qore streams through React's external store contract, so the stream remains the source of truth while React renders the view.
...
| React adapter | Preview | `packages/react` bridges Qore streams through `useSyncExternalStore` |
