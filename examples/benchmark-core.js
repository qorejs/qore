...
function renderQoreShell(answer) {
    return h('section', { className: 'bench-frame' }, h('header', { className: 'bench-head' }, h('span', null, 'stream = signal'), h('span', null, `${benchmarkChunks.length} chunks`)), h('div', { className: 'bench-feed' }, benchmarkHistory.map((message) => h('article', { className: ['bench-msg', message.role] }, h('strong', null, message.role === 'assistant' ? 'Qore' : 'You'), h('p', null, message.body))), h('article', { className: ['bench-msg', 'assistant', 'live'] }, h('strong', null, 'Qore'), h('p', null, text(() => answer())))), h('footer', { className: 'bench-foot' }, h('span', null, 'composer'), h('span', null, 'one transcript')));
}
...
export async function runBenchmarkSuite(options = {}) {
    ...
    const results = [
        summarizeRuns('qore', 'Qore stream = signal', 'Mount the transcript once, then let a live stream advance the same text node.', qoreRuns),
        summarizeRuns('snapshot', 'Snapshot rerender baseline', 'Rebuild the transcript shell as a string on every chunk, like a snapshot-first chat loop.', snapshotRuns)
    ];
