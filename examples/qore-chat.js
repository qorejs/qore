...
createApp(() => {
    ...
    return {
        ...
        view: () => h('main', { className: 'shell' }, h('div', { className: 'masthead' }, h('a', { className: 'home-link', href: '../index.html' }, 'Back To Landing'), h('span', { className: 'demo-badge' }, 'Focused Demo')), h('section', { className: 'hero' }, h('p', { className: 'eyebrow' }, 'Qore / Streaming Response'), h('h1', null, '让数据像水流一样进入你的 UI'), h('p', { className: 'lede' }, 'stream 是数据流动的方式, signal 是 UI 响应变化的方式。在 Qore 里, 它们是同一个 primitive。')), h('section', { className: 'panel' }, h('div', { className: 'feed' }, list(messages, (message) => {
            ...
            return h('article', { className: `message ${message.role}` }, h('div', { className: 'meta' }, h('strong', null, message.role === 'assistant' ? 'Qore' : 'You'), h('span', { className: 'state' }, text(() => live() ? 'typing...' : message.role === 'assistant' ? 'done' : 'sent'))), h('div', { className: 'markdown', innerHTML: () => renderMarkdown(body()) }));
        })), h('label', { className: 'composer' }, h('span', null, 'Prompt'), h('input', {
            value: draft,
