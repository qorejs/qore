...
export function createErrorBoundary() {
  ...
  return {
    state,
    handleError: (error: Error) => { state({ hasError: true, error }); },
...
export async function retry<T>(
  fn: () => Promise<T>,
  options: { maxRetries?: number; delay?: number; backoff?: number; onError?: (error: Error, attempt: number) => void } = {}
): Promise<T> {
  const { maxRetries = 3, delay = 1000, backoff = 2, onError } = options;
...
export function withErrorBoundary<T extends Record<string, any>>(
  ...
  options: { fallback?: (error: Error, reset: () => void) => any; onError?: (error: Error) => void } = {}
): (props: T) => any {
  return (props: T) => {
    ...
    try {
      ...
      options.onError?.(err);
      if (options.fallback) return options.fallback(err, boundary.reset);
      return null;
    }
  };
}
